        ...
        if (Cache[path] == null)
        {
            ...
            Cache.Add(path, text,
                      ...
                      (string key, object value, CacheItemRemovedReason reason) =>
                          GetFileContentInCache(key, callerPath));

            //
            // Marca o arquivo como modificado assim o servidor não retornará um http 304 
            //
            System.IO.File.SetLastWriteTime(callerPath, DateTime.UtcNow);
        }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        foreach (var filename in files)
            if (!String.IsNullOrEmpty(filename))
                Response.Write(GetFileContentInCache(basePath + filename.Trim() + extension, Request.PhysicalPath));
