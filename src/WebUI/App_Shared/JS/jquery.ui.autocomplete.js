    ...
    $.widget("ui.autocomplete", {
        ...
        _init: function() {
            ...
            $.extend(this.options, {
                ...
                inputClass: this.options.inputClass || "ui-autocomplete-input",
                resultsClass: this.options.resultsClass || "ui-widget ui-widget-content ui-autocomplete-results",
                loadingClass: this.options.loadingClass || "ui-autocomplete-loading",
                minChars: this.options.minChars || 1,
                ajaxDelay: this.options.ajaxDelay || 400,
                localDelay: this.options.localDelay || 10,
                // TODO replace these three with cacheMatch option
                matchCase: this.options.matchCase || false,
                matchSubset: this.options.matchSubset || true,
                matchContains: this.options.matchContains || false,
                // TODO verify usefulness
                cacheLength: this.options.cacheLength || 10,
                scrollMax: this.options.scrollMax || 150,
                noScrollMax: this.options.noScrollMax || 10,
                mustMatch: false,
                // TODO replace
                extraParams: {},
                // TODO remove
                ...
                autoFill: this.options.autoFill || false,
                width: 0,
                // TODO remove
                ...
                highlight: this.options.highlight || function(value, term) {
                    if (value == undefined) return value;
                    return value.replace(new RegExp("(?![^&;]+;)(?!<[^<>]*)(" + term.replace(/([\^\$\(\)\[\]\{\}\*\.\+\?\|\\])/gi, "\\$1") + ")(?![^<>]*>)(?![^&;]+;)", "gi"), "<span class='match'>$1</span>");
                },
                scroll: this.options.scroll || true,
                scrollHeight: this.options.scrollHeight || 180
            });
            ...
            //remove onchange 
            input.onchanged = input.onchange;
            input.onchange = null;
            ...
            function request(term, success, failure) {
                ...
                if (data && data.length) {
                    ...
                    $.ajax({
                        ...
                        dataType: options.dataType || "json",  // not "json" we'll parse
                        contentType: "application/json",
                        timeout: 10000,

                        url: options.url,
                        data: '{"q": "' + query + '", "limit": ' + (options.max || 10) + '}',
    ...
    $.ui.autocomplete.select = function(options, input, select, config) {
        ...
        return {
            ...
            show: function() {
                ...
                element.css({
                    width: typeof options.width == "string" || options.width > 0 ? options.width : ($(input).width() + 25),
