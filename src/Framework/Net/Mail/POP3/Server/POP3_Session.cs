...
using InfoControl.Net.Mail.AUTH;
...
namespace InfoControl.Net.Mail.POP3
{
	...
	/// Pop33 Session.
	/// </summary>
	public class Pop3Session : ISocketServerSession
	{		
		private BufferedSocket m_pSocket        = null;  // Referance to client Socket.
		private Pop3Server    m_pServer        = null;  // Referance to Pop33 server.
		...
		private Pop3Messages  m_Pop3Messages  = null;		
	...
	//	private _LogWriter     m_pLogWriter     = null;
		...
		//		m_pLogWriter.AddEntry("//----- Sys: 'Session:'" + this.SessionID + " added " + DateTime.Now);
			...
			try{
				...
				if(m_pServer.OnValidate_IpAddress(this.LocalEndPoint,this.RemoteEndPoint)){
					...
					m_pSocket.SendLine("+OK " + m_pServer.HostName + " Pop33 Server ready " + m_MD5_prefix);
		...
		/// </summary>
		/// <param name="PopcommandTxt">POP3 command text.</param>
		/// <returns>Returns true,if session must be terminated.</returns>
		private bool SwitchCommand(string Pop3commandTxt)
		{
			//---- Parse command --------------------------------------------------//
			string[] cmdParts = Pop3commandTxt.TrimStart().Split(new char[]{' '});
			string Pop3command = cmdParts[0].ToUpper().Trim();
			string argsText = Core.GetArgsText(Pop3commandTxt,Pop3command);
			//---------------------------------------------------------------------//
		...
		private void USER(string argsText)
		{
			...
			NOTE:
				If the Pop33 server responds with a positive
				status indicator ("+OK"), then the client may issue
				either the PASS command to complete the authentication,
				or the QUIT command to terminate the Pop33 session.
		...
		{	
			...
			NOTE:
				When the client issues the PASS command, the Pop33 server
			...
			*/
			...
			if(param.Length == 1){
				...
				if(aArgs.Validated){					
					...
					m_pServer.OnGetMessagesInfo(this,m_Pop3Messages);
		...
		{	
			...
			*/
			...
			m_pSocket.BeginSendLine("+OK " + m_Pop3Messages.Count.ToString() + " " + m_Pop3Messages.GetTotalMessagesSize(),new SocketCallBack(this.EndSend));			
		}
		...
		{	
			...
			NOTE:
				If an argument was given and the Pop33 server issues a
				positive response with a line containing information for
				that message.

				If no argument was given and the Pop33 server issues a
			...
			*/
			...
			if(argsText.Length == 0){
				string reply = "+OK " + m_Pop3Messages.Count.ToString() + " messages\r\n";

				// Send message number and size for each message
				foreach(Pop3Message msg in m_Pop3Messages.ActiveMessages){
				...
				if(param.Length == 1){
					...
					if(Core.IsNumber(param[0])){
						...
						if(m_Pop3Messages.MessageExists(messageNr)){
							Pop3Message msg = m_Pop3Messages[messageNr];
		...
		private void RETR(string argsText)
		{
			...
			NOTE:
				If the Pop33 server issues a positive response, then the
			...
			Example:
				C: RETR 1
				S: +OK 120 octets
				S: <the Pop33 server sends the entire message here>
				S: .
		...
		{	
			...
			NOTE:
				The Pop33 server marks the message as deleted.  Any future
				reference to the message-number associated with the message
				in a Pop33 command generates an error.  The Pop33 server does
				not actually delete the message until the Pop33 session
			...
			*/
			...
			if(argsText.Length > 0 && param.Length == 1){
				...
				if(Core.IsNumber(param[0])){
					...
					if(m_Pop3Messages.MessageExists(nr)){
						Pop3Message msg = m_Pop3Messages[nr];
		...
		private void NOOP()
		{
			/* RFC 1939 5. NOOP
			NOTE:
				The Pop33 server does nothing, it merely replies with a
				positive response.
			*/
		...
		private void RSET()
		{
			/* RFC 1939 5. RSET
			Discussion:
				If any messages have been marked as deleted by the Pop33
				server, they are unmarked.  The Pop33 server then replies
		...
		private void QUIT()
		{
			/* RFC 1939 6. QUIT
			NOTE:
				The Pop33 server removes all messages marked as deleted
			...
			*/					
			...
			m_pSocket.SendLine("+OK Pop33 server signing off");
		//	m_pSocket.BeginSendLine("+OK Pop33 server signing off",null);			
		}
		...
		{		
			...
			Examples:
				C: TOP 1 10
				S: +OK
				S: <the Pop33 server sends the headers of the
		...
		private void UIDL(string argsText)
		{
			...
			NOTE:
				If an argument was given and the Pop33 server issues a positive
				response with a line containing information for that message.

				If no argument was given and the Pop33 server issues a positive
				response, then the response given is multi-line.  After the
				initial +OK, for each message in the maildrop, the Pop33 server
		...
		private void APOP(string argsText)
		{
			...
			NOTE:
				A Pop33 server which implements the APOP command will
				include a timestamp in its banner greeting.  The syntax of
				the timestamp corresponds to the `msg-id' in [RFC822], and
				MUST be different each time the Pop33 server issues a banner
				greeting.
				
			Examples:
				S: +OK Pop33 server ready <1896.697170952@dbc.mtview.ca.us>
			...
			/* Rfc 1734
				...
				AUTH mechanism
					...
					Discussion:
						...
						exchange, the Pop33 server issues a positive response and
						the Pop33 session enters the TRANSACTION state.
						
				Examples:
							S: +OK Pop33 server ready
							C: AUTH KERBEROS_V4
							S: + AmFYig==
		...
		private void CAPA(string argsText)
		{
			/* rfc 2449 5.  The CAPA Command
				...
				The Pop33 CAPA command returns a list of capabilities supported by the
		...
		{		
			...
			*/
			m_Pop3Messages.ResetDeleteFlags();
		}
		...
		private void Update()
		{
			...
			NOTE:
				...
				the Pop33 session enters the UPDATE state.  (Note that if the client
				issues the QUIT command from the AUTHORIZATION state, the Pop33
				session terminates but does NOT enter the UPDATE state.)

				If a session terminates for some reason other than a client-issued
				QUIT command, the Pop33 session does NOT enter the UPDATE state and
			...
			*/
			...
			if(m_Authenticated){
				...
				foreach(Pop3Message msg in m_Pop3Messages.Messages){
